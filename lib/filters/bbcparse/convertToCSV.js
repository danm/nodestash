    ...
    for (let row in flat) {
        if (i === 0) {
            if (flat[row].value === null) {
                ...
                line += mysql_real_escape_string(flat[row].value);
            } else if (flat[row].value !== undefined && flat[row].type === 'int') {
                let num = Number(flat[row].value);
                if (isNaN(num)) {
                    line += '0';
                } else {
            ...
            if (flat[row].value !== undefined && flat[row].type === 'string' && flat[row].value === null) {
                line += ', NULL';
            } else if (flat[row].value !== undefined && flat[row].type === 'string' && flat[row].value !== null) {
                line += ", " + mysql_real_escape_string(flat[row].value);
            } else if (flat[row].value !== undefined && flat[row].type === 'int') {
                let num = Number(flat[row].value);
                if (isNaN(num)) {
            ...
            } else if (flat[row].value !== undefined && flat[row].type === 'float') {
                let num = Number(flat[row].value);
                if (isNaN(num)) {
                    line += ', ';
                } else {
