...
module.exports = (line) => {
  ...
  if (line.type === 'view') {
    fs.appendFileSync('./tmp/redshift-view.csv', `${redshiftClean.join(', ')}\n`);
  } else if (line.type === 'hidden' && line.video && line.video.vpid) {
    fs.appendFileSync('./tmp/redshift-video.csv', `${redshiftClean.join(', ')}\n`);
  } else {
    // fs.appendFileSync('./tmp/redshift-hidden.csv', `${redshiftClean.join(', ')}\n`);
