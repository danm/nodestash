    ...
    return new Promise(async (resolve, reject) => {
        ...
        if (tempLine.bbc.cpsid) {
            ...
            try {
                ...
                _this.abacus.c.a(`Error getting CPS meta data: ${e.message}`, 2, 'bbc-parse');
            } 
        ...
        if (_this.abacus.config.output.mongo && _this.abacus.config.output.mongo.active === true) {
            if (tempLine.bbc.cpsid && tempLine.type === 'view') {
                fs.appendFileSync('./tmp_mongo.ndjson', `${JSON.stringify(tempLine)} \n`);
            }
        }

        if (_this.abacus.config.output.elastic && _this.abacus.config.output.elastic.active === true) {
            if (tempLine.bbc.cpsid && tempLine.type === 'view') {
                elasticSearch(tempLine);
            }
        }

        if (_this.abacus.config.output.redshift && _this.abacus.config.output.redshift.active === true) {
            redshift(tempLine)
        }
