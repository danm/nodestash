...
const mapResult = (contentStore) => {
  return contentStore.map((row) => {
    return {
      countername: row.iStatsCounterName || row.assetId,
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      if (contentStore === undefined) {
        ...
        abacus.c.a(`Error getting ${cpsid}: setting empty object i  cache`, 2, 'bbcparse/getmetadata/contentstore/noresult');
        ...
        abacus.c.a(`Received ${cpsid}: setting cache`, 1, 'bbcparse/getmetadata/contentstore/result');
