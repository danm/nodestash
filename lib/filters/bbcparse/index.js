...
const elasticSearchVideo = require('./elasticSearchVideo');
    ...
    return new Promise(async (resolve, reject) => {
        ...
        if (tempLine.bbc.cpsid) {
            ...
            if (meta !== undefined) {
                ...
                tempLine.bbc.publishedDate = meta.publishedDate;
            }
        }
        ...
        if (
            (tempLine.ns_jspageurl && tempLine.ns_jspageurl.includes('?app=')) &&
        ...
        // if view comes from the app, attribute to bbc
        if (tempLine.bbc.platform === 'mobile-app') {
          tempLine.rp.type = 'bbc';
          tempLine.rp.name = 'mobile-app';
        }

        // SPORT
        // Sports app is a cocoa shell around a html site, the only way to identify it is the app is the is_app flag
        if (tempLine.bbc.site === '/sport' && tempLine.bbc.platform === 'responsive' && line.is_app === '1') {
            tempLine.bbc.platform = 'mobile-app';
            tempLine.app.family = 'BBCSport';
            tempLine.rp.type = 'bbc';
            tempLine.rp.name = 'mobile-app';
        }
        ...
        if (line.bbc_hid) {
            let bbc_hid;
            tempLine.user.hid = line.bbc_hid; 
            try {
        ...
        // last chance to bucket direct tragffic
        if (
            tempLine.rp.type === 'direct' ||
            tempLine.rp.type === 'Direct' ||
            tempLine.rp.type === 'email' ||
            tempLine.rp.type === 'news' ||
            tempLine.rp.type === 'uknown'
        )
            {
            // we want to attribute direct traffic to social under dark social
            tempLine.rp = {
                type: 'social',
                name: 'dark-social',
                page: 'direct',
        ...
        if (_this.abacus.config.output.s3.mongo) {
            if (tempLine.bbc.cpsid && tempLine.type === 'view') {
                fs.appendFileSync('./tmp/mongo.ndjson', `${JSON.stringify(tempLine)} \n`);
            }
        }

        if (_this.abacus.config.output.s3.elasticVideo) {
          if (tempLine.bbc.cpsid && tempLine.video) {
            elasticSearchVideo(tempLine);
          }
        }

        if (_this.abacus.config.output.s3.elastic) {
            if (tempLine.bbc.cpsid && tempLine.type === 'view') {
        ...
        if (
            _this.abacus.config.output.s3.redshiftHidden ||
            _this.abacus.config.output.s3.redshiftVideo ||
            _this.abacus.config.output.s3.redshiftViews) {
            redshift(tempLine);
        }
